...
class _FlatModelStore extends LocalModelStore {
  _FlatModelStore(super.directory);

  @override
  String bundleDirFor(ModelEntry entry) => directory.path;
}
...
class _FakeWorkerEngine implements AsrEngine {
  ...
  Future<void> _record(String op) =>
      File(_config.logPath)
...
void main() {
  ...
  List<String> log(String behavior) =>
      File('${logDir.path}${Platform.pathSeparator}$behavior.log')
          .readAsLinesSync();

  test(
    'serial command order, worker-side construction, session reuse',
    () async {
      final e = engine('normal');
      // 'construct' is recorded by the builder on the worker's first command.
      expect(
        await e.capabilities(),
        isA<EngineCapabilities>().having(
          (c) => c.available,
          'available',
          isTrue,
        ),
      ...
      final first = await e
          .transcribe(const TranscribeRequest(audioPath: 'a.wav'))
          .toList();
      final second = await e
          .transcribe(const TranscribeRequest(audioPath: 'b.wav'))
          .toList();
  ...
  test(
    'engine errors end the pending call and leave the worker usable',
    () async {
  ...
  test('cancel errors the in-flight call once it lands, refuses later ones, '
      'and load clears the flag', () async {
    final e = engine('slow');
    await e.load(const EngineModelSpec(path: 'x'), Backend.cpu);
    ...
    expect(events, [
      isA<TranscribeProgress>(),
      isA<EngineCancelledException>(),
    ]);
  ...
  test('planVad is not gated by a prior cancel, but transcribe is', () async {
  ...
  test(
    'dispose before spawn completes without ever starting a worker',
    () async {
  ...
  test('identical load reuses the session; any spec field or backend change reloads', () async {
    final e = engine('normal');
  ...
  test('trusted catalog identity fields each invalidate the session', () async {
    final e = engine('normal');
    final dir = Directory.systemTemp.createTempSync('worker-model-identity');
    addTearDown(() => dir.deleteSync(recursive: true));
    final store = _FlatModelStore(dir);
    int loads() => log('normal').where((l) => l.startsWith('load:')).length;

    EngineModelSpec spec({
    ...
    await e.load(spec(), Backend.cpu);
    await e.load(spec(), Backend.cpu);
    expect(loads(), 1);

    await e.load(spec(bundleId: 'bundle-b'), Backend.cpu);
    expect(loads(), 2);
    await e.load(spec(bundleId: 'bundle-b', modelSize: 3), Backend.cpu);
    expect(loads(), 3);
    await e.load(
      spec(
        bundleId: 'bundle-b',
  ...
  test('a failed load is never cached as loaded', () async {
    ...
    expect(log('fail-load').where((l) => l.startsWith('load:')).length, 2);
    await e.dispose();
  });
  ...
  test(
    'registry-backed sherpa-mnn worker probes honestly on this host',
    () async {
      // This exercises construction inside the isolate as well as reply/error
      // transport. A host without the Android library must report unavailable;
      // it may never substitute a fake capability or transcript.
      final e = EngineRegistry().createAsr('sherpa-mnn');
      addTearDown(e.dispose);

      final caps = await e.capabilities().timeout(const Duration(seconds: 30));
      if (caps.available) {
        expect(caps.unavailableReason, isNull);
        expect(caps.backends, contains(Backend.cpu));
        ...
        await expectLater(
          e.load(
            const EngineModelSpec(path: 'missing.mnn', family: 'sensevoice'),
            Backend.cpu,
          ),
          throwsA(isA<EngineUnavailableException>()),
        );
